...
/* ─── Modal détail / édition ─────────────────────────────────────────────────
   Affiche TOUS les champs d'un risque — y compris les descriptions EPC/ORG/EPI
   invisibles dans le tableau. Tous les champs sont modifiables ; la sauvegarde
   appelle directement Supabase via le callback onSave.
   ─────────────────────────────────────────────────────────────────────────── */
function RisqueDetailModal({ row, onClose, onSave, listeUT, listeFamilles, listePersonnes, isDark, p, isSaving }) {
  const toArr = v => Array.isArray(v) ? v : (v ? String(v).split(' / ').filter(Boolean) : []);
  const [form, setForm] = useState({ ...row, personnes_exposees: toArr(row.personnes_exposees) });

  const ci     = Number(form.gravite || 1) * Number(form.probabilite || 1);
  const coeff  = getCoefficient(form.a_mesure_epc, form.a_mesure_orga, form.a_mesure_epi);
  const cr     = Math.max(1, Math.round(ci * coeff));
  ...
  const handleSave = () => {
    onSave({
      ...
      personnes_exposees: Array.isArray(form.personnes_exposees)
        ? form.personnes_exposees.join(' / ')
        : form.personnes_exposees,
    });
  };
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.78)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
    >
      <div style={{ background: isDark ? '#0F1729' : '#F8FAFC', border: `1px solid ${p.border}`, borderRadius: 16, width: '100%', maxWidth: 860, maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 40px 100px rgba(0,0,0,0.7)' }}>

        {/* Header */}
        <div style={{ padding: '16px 22px', borderBottom: `1px solid ${p.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: p.text1, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Eye size={16} color="#F59E0B"/>
              {form.danger || 'Détail du risque'}
            </div>
            <p style={{ fontSize: 11, color: p.text4, marginTop: 2 }}>
              {form.unite_travail}{form.famille_risque ? ` · ${form.famille_risque}` : ''}
            </p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: p.text4, padding: 6 }}><X size={18}/></button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '18px 22px' }}>
          ...
          {/* ② Identification */}
          <div style={{ background: p.whiteFaint2, border: `1px solid ${p.border}`, borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: '#F59E0B', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>② Identification du risque</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label style={lbl}>Danger précis *</label>
                <input type="text" value={form.danger || ''} onChange={e => setForm({ ...form, danger: e.target.value })} className="input-modern"/>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.dommage_potentiel || ''} onChange={e => setForm({ ...form, dommage_potentiel: e.target.value })} placeholder="Ex : fracture, brûlure..." className="input-modern"/>
              </div>
              <div className="md:col-span-2">
                <label style={lbl}>Personnes exposées</label>
                <div style={{ border: `1px solid ${p.border}`, borderRadius: 9, background: p.bgInput, padding: '6px 8px', minHeight: 42, display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  ...
                  <select value="" onChange={e => { const v = e.target.value; if (v && !(form.personnes_exposees || []).includes(v)) setForm({ ...form, personnes_exposees: [...(form.personnes_exposees || []), v] }); }}
                    style={{ border: 'none', background: 'transparent', color: p.text3, fontSize: 13, cursor: 'pointer', outline: 'none', flex: 1, minWidth: 120 }}>
          ...
          <div style={{ background: p.whiteFaint2, border: `1px solid ${p.border}`, borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <div key={f.key}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[1, 2, 3, 4].map(v => {
                      ...
                      return (
                        <button key={v} title={f.tips[v - 1]} onClick={() => setForm({ ...form, [f.key]: v })}
              ...
              <div>
                ...
                <div style={{ background: ciInfo.bg, border: `1.5px solid ${ciInfo.border}`, borderRadius: 8, padding: '10px 14px', textAlign: 'center' }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: ciInfo.color }}>{ci}</div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: ciInfo.color, marginTop: 2 }}>{ciInfo.label}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: p.whiteFaint2, border: `1px solid ${p.border}`, borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { key: 'a_mesure_epc',  descKey: 'mesures_epc',  label: 'EPC — Protection Collective',   color: '#3B82F6', coeff: '×0.50', ph: 'Garde-corps, aspiration, écran...' },
                { key: 'a_mesure_orga', descKey: 'mesures_orga', label: 'ORG — Organisationnelle',       color: '#8B5CF6', coeff: '×0.70', ph: 'Procédure, permis, habilitation...' },
                { key: 'a_mesure_epi',  descKey: 'mesures_epi',  label: 'EPI — Protection Individuelle', color: '#F59E0B', coeff: '×0.80', ph: 'Casque, harnais, lunettes...' },
              ].map(m => (
                <div key={m.key}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <span onClick={() => setForm({ ...form, [m.key]: !form[m.key] })}
            ...
            <div style={{ marginTop: 14, background: crInfo.bg, border: `1.5px solid ${crInfo.border}`, borderRadius: 8, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, color: p.text3 }}>CI={ci} × Cr=<strong style={{ color: p.text1 }}>{coeff}</strong> =</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 30, fontWeight: 900, color: crInfo.color }}>{cr}</span>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: crInfo.color }}>{crInfo.label}</div>
          ...
          {/* ⑤ Action planifiée */}
          <div style={{ background: p.whiteFaint2, border: `1px solid ${p.border}`, borderRadius: 10, padding: '14px 16px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: isDark ? '#34D399' : '#047857', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>⑤ Action planifiée</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-3">
                <label style={lbl}>Description de l'action</label>
                <input type="text" value={form.action_preventive || ''} onChange={e => setForm({ ...form, action_preventive: e.target.value })} placeholder="Mesure préventive..." className="input-modern"/>
              </div>
              <div>
        ...
        {/* Footer */}
        <div style={{ padding: '12px 22px', borderTop: `1px solid ${p.border}`, display: 'flex', justifyContent: 'flex-end', gap: 10, flexShrink: 0, background: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.02)' }}>
          <button onClick={onClose} className="btn-secondary">Annuler</button>
          <button onClick={handleSave} disabled={isSaving} className="btn-primary" style={{ background: '#F59E0B', boxShadow: '0 0 16px rgba(245,158,11,0.3)' }}>
...
export default function RegistreDUERP({ onNavigateToPdca }) {
  ...
  // Sous-onglets : 'registre' | 'matrice' | 'historique'
  ...
  /* ── Sauvegarde depuis la modal ─────────────────────────────────────────── */
  const handleModalSave = async (updatedData) => {
    updateRow(updatedData.id, updatedData);
    await saveRowDirect(updatedData);
    setDetailRow(null);
  };
    ...
    <div className="glass-panel p-5 animate-fade-up">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold flex items-center gap-2" style={{ color: p.text1 }}>
          Matrice de criticité — 4 niveaux
        </h3>
      </div>
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: `1px solid ${p.border}`, paddingBottom: 0 }}>
        {[
          { id: 'registre',   label: 'Registre',   count: risques.filter(r => !r.archived_at).length },
          { id: 'matrice',    label: 'Matrice' },
          { id: 'historique', label: 'Historique',  count: risques.filter(r => r.archived_at).length },
        ].map(tab => {
          ...
          return (
            <button
              ...
              style={{
                ...
                border: 'none', borderBottom: `2px solid ${isActive ? '#F59E0B' : 'transparent'}`,
                background: 'transparent',
                color: isActive ? '#F59E0B' : p.text3,
                display: 'flex', alignItems: 'center', gap: 6,
                transition: 'all 0.15s', marginBottom: -1,
              }}>
              {tab.label}
              {tab.count !== undefined && (
                <span style={{ fontSize: 10, fontWeight: 800, minWidth: 18, height: 18, borderRadius: 100, background: isActive ? 'rgba(245,158,11,0.2)' : p.whiteFaint2, color: isActive ? '#F59E0B' : p.text4, border: `1px solid ${isActive ? 'rgba(245,158,11,0.4)' : p.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>
                  {tab.count}
                </span>
              )}
            </button>
      ...
      {/* ── Onglet MATRICE ──────────────────────────────────────────────────── */}
      {activeTab === 'matrice' && <MatriceRisques />}

      {/* ── Onglet REGISTRE — KPIs ──────────────────────────────────────────── */}
      {activeTab === 'registre' && (
        <>
            ...
            <div className="glass-panel p-6 animate-fade-up" style={{ border: '1px solid rgba(245,158,11,0.3)' }}>
              ...
              <div style={{ background: p.whiteFaint2, border: '1px solid ' + p.border, borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-3">
                    <label style={lbl}>Description de l'action</label>
                    <input type="text" value={form.action_preventive} onChange={e => setForm({ ...form, action_preventive: e.target.value })} placeholder="Ex: Installer un revêtement antidérapant..." className="input-modern"/>
                  </div>
                  <div>
                    <label style={lbl}>Pilote</label>
        ...
        <div className="glass-panel">
          {loading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="font-bold" style={{ color: p.text1 }}>
                {activeTab === 'historique' ? 'Aucun risque archivé.' : risques.length === 0 ? 'Aucun risque identifié.' : 'Aucun risque pour ces filtres.'}
              </p>
              {activeTab === 'registre' && risques.length === 0 && <p style={{ color: p.text3, fontSize: 13, marginTop: 6 }}>Cliquez sur "Identifier un risque" pour commencer votre évaluation.</p>}
            </div>
          ) : (
                      ...
                      <tr key={row.id} style={{ borderLeft: `3px solid ${leftBorder}` }}>
                        ...
                        <td>
                          <select value={row.unite_travail || listeUT[0]}
                            onChange={e => { const v = e.target.value; updateRow(row.id, { unite_travail: v }); saveRowDirect({ ...row, ...risquesRef.current.find(r => r.id === row.id), unite_travail: v }); }}
                        ...
                        <td className="text-center">
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                            {[
                              ...
                              <button key={m.key}
                                onClick={() => { const cur = risquesRef.current.find(r => r.id === row.id) || row; const val = !cur[m.key]; const epc = m.key === 'a_mesure_epc' ? val : cur.a_mesure_epc; const orga = m.key === 'a_mesure_orga' ? val : cur.a_mesure_orga; const epi = m.key === 'a_mesure_epi' ? val : cur.a_mesure_epi; const upd = { ...cur, [m.key]: val, criticite_resid: calcCR(cur.gravite, cur.probabilite, epc, orga, epi), coefficient_reducteur: getCoefficient(epc, orga, epi) }; updateRow(row.id, { [m.key]: val }); saveRowDirect(upd); }}
                        ...
                        <td className="text-center">
                          <div style={{ display: 'flex', gap: 2, justifyContent: 'center', alignItems: 'center' }}>
                            {saving === row.id
                              ...
                              : <>
                                  {/* Bouton œil — ouvre le panneau de détail complet */}
                                  <button
                                    onClick={() => setDetailRow(row)}
                                    title="Voir / modifier tous les champs"
                                  ...
                                  {showArchive
                                    ...
                                    : <WriteOnly><button onClick={() => archiveRow(row.id)} title="Archiver" style={{ color: p.text4, background: 'none', border: 'none', cursor: 'pointer', padding: 5 }} className="hover:text-amber-400"><Archive size={14}/></button></WriteOnly>
                                  }
                                </>
                            }
                          </div>
